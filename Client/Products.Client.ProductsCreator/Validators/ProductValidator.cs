namespace Products.Client.ProductsCreator.Validators
{
    ...
    internal sealed class ProductValidator : AbstractValidator<Product>
    {
        ...
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("Поле 'Наименование' должно быть заполнено");
            RuleFor(x => x.Name).Length(1, 100).WithMessage("Длина поля 'Наименование' должно быть в диапазоне от 1 до 100");
